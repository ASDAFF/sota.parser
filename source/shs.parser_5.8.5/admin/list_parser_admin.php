...
while($arRes = $rsData->NavNext(true, "f_")):
    ...
    if($f_T=="S")
    {
        ...
        if(!$startAgent)
            ...
            $row->AddViewField("NAME", '<span title="'.GetMessage("parser_act_start_agent").'">'.$f_NAME.'</span>');
    }
    ...
    if(!$startAgent)
        ...
        $row->AddViewField("RSS", '<span title="'.GetMessage("parser_act_start_agent").'">'.$f_RSS.'</span>');
    ...
    if($POST_RIGHT=="W" && $f_T=="P" && !$startAgent)
        $arActions[] = array(
            "ICON"=>"edit",
            "DEFAULT"=>true,
...
$oFilter->End();
?>
<input type="hidden" name="parent" value="<?=htmlspecialcharsbx($_REQUEST["parent"])?>" />
</form>
