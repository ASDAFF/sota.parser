...
//            }
    ...
    protected function parseCatalogXlsProducts()
    {
        ...
        if(is_array($this->settings['catalog_level']))
        ...
        for($key=0;$key<$listCount;$key++)
        {
            ...
            $startRow = $this->settings['list'][$key]['first_item'] ? $this->settings['list'][$key]['first_item'] : 0;
            $startRow++;
            ...
            while($startRow<=$totalRows){
                ...
                if(is_callable(array($worksheet, 'getRealHighestRow')))
                    $heghestRow = intval($worksheet->getRealHighestRow());
                elseif($worksheet->getHighestDataRow() > $heghestRow)
                    $heghestRow = intval($worksheet->getHighestDataRow());
...
//        foreach($arRes as $el)
    ...
    protected function parseAdditionalStoresXLS(&$el){
    ...
    public function parseCatalogPricePreviewXls(&$el)
    {
    ...
    {
        if(isset($this->settings["prices_preview"]) && !empty($this->settings["prices_preview"]))
        {
    ...
    protected function parseCatalogDetailMorePhotoXls(&$el)
    {
        if($this->settings["catalog"]["more_image_props"])
        {
            ...
            if($this->settings["catalog"]["delimiter_imgs"] == '')
    ...
    public function getCalculatedValue($val) {
        try
        {
            if($this->params['ELEMENT_NOT_LOAD_FORMATTING']=='Y')
...
class SotbitXlsParserUtils  extends SotbitXlsParser {
    ...
    public function printHtmlTable($arTable = array(), $useNum = false, $params = false, $blockId = null, $listnum, $hybrid = null){
        echo '<table class="preview-xls-data" style="border-collapse: collapse;width: 100%;">';
        if($listnum==0)
            $this->printHtmlTableSettingRow($params, count($arTable[0]), $blockId, $useNum, $hybrid);
    ...
    public function printHtmlTableSettingRow($params, $count, $blockId = null, $useNum = false, $hybrid = false){
        if($params){
            ...
            for($i=0;$i<$count;$i++){
                echo '<td><div>';
                $this->printSettingSelect('SETTINGS[index_prop]['.$i.']',$i, $blockId, $hybrid);
    ...
    public function printSettingSelect($nameSelect, $colNum, $blockId = null, $hybrid = false){
        $fields = $this->getFields($blockId, false, $hybrid);
    ...
    public function getFields($IBLOCK_ID, $offers = false, $hybrid)
    {
            $aFields = array();
            ...
            if($hybrid){
                $aFields[$IBLOCK_ID]['element']['items']["IE_DETAIL_LINK"] = GetMessage("XLS_IE_FI_DETAIL_LINK");
            }
